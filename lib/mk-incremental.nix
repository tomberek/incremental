...
in
{
  ...
  # find, not a fixed-depth glob: a tool's own cache dir (e.g.
  # ccache's hex-keyed subdirs) can nest arbitrarily deep depending on
  # how big the build is — a shallow glob silently leaves deeper refs
  # un-nuked, which showed up as a real "cycle detected ... in the
  # references of output 'bin' from output 'incremental'" on a big
  # enough package. config.cache is excluded: unlike a cache-tool's
  # own manifest, mkIncrementalAutotoolsPackage's config.cache is
  # autoconf's own check results and is *supposed* to keep real store
  # paths (e.g. the discovered path to `mkdir`) valid across builds —
  # nuking it corrupts those paths, and the next ./configure re-run
  # propagates the corruption straight into a freshly generated
  # Makefile (confirmed: "mkdir: No such file or directory" pointing
  # at a nuke-refs placeholder hash).
  nukeScript =
