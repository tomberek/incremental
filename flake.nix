    ...
    packages = builtins.mapAttrs (system: pkgs: rec {
      hello-ccache = (pkgs.hello.override { stdenv = pkgs.ccacheStdenv; }).overrideAttrs (old: {
        ...
        postPatch = ''
          ...
          mkdir empty
          cp -r ${inputs.cache.packages.${system}.hello-ccache.incremental or "empty"} $incremental
          chmod -R +w $incremental
        '';
        postInstall =
          (old.postInstall or "")
          + (
            if inputs.cache ? packages then
              ''
      ...
      golang = pkgs.buildGoModule {
        ...
        postConfigure = ''
          mkdir empty
          cp -r ${inputs.cache.packages.${system}.golang.incremental or "empty"} $incremental
          chmod -R +w $incremental
      ...
      zig = pkgs.stdenvNoCC.mkDerivation {
        ...
        preConfigure = ''
          mkdir empty
          cp -r ${inputs.cache.packages.${system}.zig.incremental or "empty"} $incremental
          chmod -R +w $incremental
